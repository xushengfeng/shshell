...
const getChunk = (output: string) => {
    const lines = parseOut(output).items;
...
describe("Styles (Color & Interaction)", () => {
    describe("ANSI Color Parsing", () => {
        it("should parse basic foreground color (8 color with bright) (31m)", () => {
            const chunk = getChunk("\x1b[31mRed\x1b[0m");
            expect(chunk).toMatchObject({ text: "Red", style: { color: "_red" } });
            const chunk2 = getChunk("\x1b[91mBrightRed\x1b[0m");
    ...
    describe("Reset & Conversion", () => {
        it("should reset styles with 0", () => {
            const lines = parseOut("\x1b[1;31mRedBold\x1b[0mNormal").items;
        ...
        it("should reset only color with 39", () => {
            const chunk = getChunk("\x1b[1;31m\x1b[39mOnlyBold");
            expect(chunk?.style).toMatchObject({
                bold: true,
                color: "_default",
            });
        ...
        it("should handle Inverse (7)", () => {
            ...
            // 在渲染层才计算实际颜色。
            const chunk = getChunk("\x1b[31m\x1b[7mTest\x1b[0m");
...
describe("Styles By txt File", () => {
    const result = parseOut(attr_example).items;
    it("check color", () => {

...
3. **Fig 自动补全**：通过动态 `import()` 加载规范文件，已外部化 `@withfig/autocomplete/dynamic`
4. **SSH 功能**：ssh2 模块功能尚在早期实现阶段，仅支持基本 exec
5. **测试覆盖**：测试文件位于 `src/renderer/sh/test/`，使用 Vitest 框架
